...
def read_chunk(f):
    ...
    chunk_length, chunk_type = struct.unpack('>I4', f.read(8))
...
width, height, bitd, colort, compm, filterm, interlacem = \
...
filtered_image_data = [
    ...
    # First scanline, first item (0) specifies filter type 0
    [0, 0, 255, 0, 255, 255, 255, 255, 255],
    # Second scanline, first item (0) specifies filter type 0
]
IDAT_data = b''.join(chunk_data for chunk_type, chunk_data in chunks
